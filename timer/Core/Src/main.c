...
//    TIM3->CNT = 0;
//    while (TIM3->CNT < (ms * 1000));
//}
//
//void TIM3_init(void) {
//    RCC->APB1ENR |= (1 << 1);         // Bật clock cho TIM3
//
//    TIM3->PSC = 16 - 1;               // 84 MHz / 84 = 1 MHz -> 1 tick = 1 µs
//    TIM3->ARR = 0xFFFFFFFF;           // Max giá trị đếm
//    TIM3->CR1 |= (1 << 0);            // Bật counter
//}
//
...
//    // Cấu hình TIM2
//    TIM2->PSC = 16 - 1;               // 1 MHz
//    TIM2->ARR = 1000 - 1;             // PWM period = 1ms (1kHz)
//
...
//
//    TIM2->CCER |= (1 << 0);           // Kích hoạt output CH1
//
//    TIM2->CR1 |= (1 << 7);            // Cho phép ARR preload
...
//    GPIOC->MODER &= ~(3 << 2); // PC1: Input
//    GPIOC->PUPDR &= ~(3 << 2); // Xóa pull-up/pull-down
//    GPIOC->PUPDR |= (1 << 2);  // PC1: Pull-up
//}
//
//// Hàm khởi tạo UART2
//void UART2_Init(void) {
...
//        sensor_state = (GPIOC->IDR >> 1) & 0x01; // Đọc bit 1 của IDR (PC1)
//
